...
@pdf_extractor_bp.route('/pdf')
def hello():
    return render_template('pdf.html')

@pdf_extractor_bp.route('/extract-pdf', methods=['POST'])
def extract_pdf():
    if 'pdfFile' in request.files:
        ...
        if not allowed_file(file.filename):
            return render_template_string("""
                <html>
                    ...
                    <body>
                        <h2>Please select a valid file type (PDF).</h2>
                        <form method="GET" action="/pdf">
                            <button type="submit">Back to extract</button>
                        </form>
                    </body>
        ...
        if file and allowed_file(file.filename):
            ...
            final_df, store_name = runAllProcess(doc, filename.split(".")[0], request.form.get('headerexists-file'), request.form.get('footerexists-file'))
            end_time = time.time()
                ...
                <html>
                    ...
                    <body>
                        <div class="complete-container">
                            ...
                            <p>Your PDF has been processed. Click the button below to download the CSV file.</p>
    ...
    elif 'pdfFolder' in request.files:
        files = request.files.getlist('pdfFolder')
        ...
        for file in files:
            ...
            if file and allowed_file(file.filename):
                ...
                doc = fitz.open(file_path)
                start_time = time.time()
                final_df, store_name = runAllProcess(doc, filename.split(".")[0], request.form.get('headerexists-folder'), request.form.get('footerexists-folder'))
                end_time = time.time()
                execution_time = end_time - start_time
                doc.close()
        ...
        return render_template_string("""
            <html>
                ...
                <body>
                    <div class="complete-container">
                        <h1>Processing Complete!!!</h1>
                        <p>Your PDFs have been processed. Click the buttons below to download the CSV files.</p>
        ...
        return render_template_string("""
            <html>
                ...
                <body>
                    <h1>Please upload a valid type of file!</h1>
                    <form method="GET" action="/pdf">
                        <button type="submit">Back to extract</button>
                    </form>
                </body>
...
@pdf_extractor_bp.route('/download_csv', methods=['GET'])
def download_csv():
